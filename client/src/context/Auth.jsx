...
const AuthProvider = ({children})=>{
    ...
    const deleteUser = async(data , userId)=>{
        try {
            setIsLoading(true);
            const response = await getAndDeleteReq(`${baseUrl}/user/${userId}` , "delete");
            // console.log("response from AuthContext! " , response?.data);
            setUser(response?.data);
            return { success: true, data: response?.data };
        } catch (error) {
    ...
    const getUsers = async()=>{
        try {
            setIsLoading(true);
            const response = await getAndDeleteReq(`${baseUrl}/user/`);
            // console.log("response from AuthContext! " , response?.data);
            setUser(response?.data);
            return { success: true, data: response?.data };
        } catch (error) {
    ...
    return(
        <AuthContext.Provider value={{user , registerUser , loginUser , logoutUser , deleteUser ,  getUser , 
        getUsers ,  error , isLoading}}>
            {children}
